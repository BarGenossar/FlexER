...
def eval_classifier(model, iterator, inference, MCML_inference, threshold=None, get_threshold=False):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            # print("yshape1: ", y.shape[1])
            # intents_num = y.shape[1]
            if inference == 'MCML':
            ...
            if 'sts-b' in taskname.lower():
                ...
                if inference == 'Multilabel':
                    y1 = y1.type_as(logits)
                    loss = nn.BCEWithLogitsLoss()(logits, y1)
                elif inference == 'MCML':
                    loss = 0.0
                    for intent in range(intents_num):
                        ...
                        tmp_loss = nn.CrossEntropyLoss()(logits[intent], tmp_y.to(device))
                    ...
                    tmp_loss = nn.BCEWithLogitsLoss()(logits[intents_num], tmp_y.to(device))
            ...
            if inference == 'MCML' and MCML_inference != 'Multilabel':
                ...
                y = y[:, int(MCML_inference)]
            Y.extend(y.numpy().tolist())
            Y_hat.extend(y_hat.cpu().numpy().tolist())
            if inference == 'MCML' and MCML_inference != 'Multilabel':
                Y_prob.extend(logits[intents_num].softmax(dim=-1).max(dim=-1)[0].cpu().numpy().tolist())
            else:
    ...
    if taskname in glue_processors:
        ...
        if num_classes <= 2 and inference != 'Multilabel':
...
def eval_on_task(epoch,
    ...
    if 'tagging' in task:
        ...
        print('Validation:')
        scalars = eval_classifier(model, valid_iter, MCML_inference)
        f1, t_f1 = 0.0, 0.0
    elif task[:5] == 'glue_':
        print('Validation:')
        scalars = eval_classifier(model, valid_iter, MCML_inference)
        ...
        if test_iter is not None:
            print('Test:')
            t_output = eval_classifier(model, test_iter, MCML_inference)
            for key in t_output:
        ...
        f1, t_f1 = 0.0, 0.0
            ...
            ['cleaning_', 'Structured', 'Textual', 'Dirty']]): # handle imbalance:
        print('Validation:')
        acc, prec, recall, f1, v_loss, th = eval_classifier(model, valid_iter, inference, MCML_inference, get_threshold=True)
        print('Test:')
        t_acc, t_prec, t_recall, t_f1, t_loss = eval_classifier(model, test_iter, inference, MCML_inference, threshold=th)
        scalars = {'acc': acc,
                   'precision': prec,
                   'recall': recall,
                   'f1': f1,
        ...
        print('Validation:')
        v_output = eval_classifier(model, valid_iter, inference, MCML_inference)

        if test_iter is not None:
            print('Test:')
            t_output = eval_classifier(model, test_iter, inference, MCML_inference)
