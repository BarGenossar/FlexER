...
class DittoDataset(SnippextDataset):
    def __init__(self,
        ...
        if type(source) is str:
            # if inference == "MCML":
            #     sents, tags_li_binary, tags_li_multiple = self.read_classification_file(source, inference)
            sents, tags_li = self.read_classification_file(source, inference)
            if size is not None:
                sents = sents[:size]
    ...
    def read_classification_file(self, path, inference):
            ...
            try:
                if len(items) == 2:
                    ...
                    if inference == "Multilabel":
                        labels_to_list = [float(label) for label in items[2]]
                        labels.append(labels_to_list)
                        # labels.append(torch.tensor(labels_to_list))
                    elif inference == "MCML":
                        labels_to_list = [float(label) for label in items[2]]
                        labels.append(labels_to_list)
                        # tags_li_multiple.append(labels_to_list)
                    else:
